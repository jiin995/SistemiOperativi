...
int main(){
	...
	for(int i=0;i<3;i++){
		res=msgrcv(id_op,&msg_op,(sizeof(msg_operandi)-sizeof(long)),P5,0);
			if(res<0){
				perror("[P5] Errore ricezione messaggio");
				exit(-1);
			}
		
		printf("[P5] Messaggio ricevuto da P3 c=%d, d=%d\n\n",
				msg_op.operandi[0],
				msg_op.operandi[1]);
		...
		msg_ris.risposta=ris;
			...
			res=msgsnd(id_ris,&msg_ris,(sizeof(msg_risposta)-sizeof(long)),0);
				if(res<0){	
					perror("[P5] Errore invio messaggio");
					exit(-1);	
				}
		printf("[P5] Risposta inviata a P3\n\n");
	}
