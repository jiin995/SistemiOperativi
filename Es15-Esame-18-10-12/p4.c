...
int main(){
	...
	for(int i=0;i<3;i++){
		res=msgrcv(id_op,&msg_op,(sizeof(msg_operandi)-sizeof(long)),P4,0);
			if(res<0){
				perror("[P4] Errore ricezione messaggio ");
				exit(-1);
			}
		printf("[P4] Ricevuto operandi da P1 g=%d h=%d \n\n",msg_op.operandi[0],msg_op.operandi[1]);
		...
		res=msgsnd(id_ris,&msg_ris,(sizeof(msg_risposta)-sizeof(long)),0);
			if(res<0){
				perror("[P4] Errore invio messaggio ");
				exit(-1);
			}
